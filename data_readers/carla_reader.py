...
class CarlaReader(Dataset):
    ...
    def __init__(self,
        ...
        if sampling_method == "sparse":
            raw_paths, pano_metadata = self.do_sparse_sampling(towns=towns,
                                                               ...
                                                               min_dist=min_dist,
                                                               max_dist=max_dist)
            self.restrided_paths = self.generate_subsequences(raw_paths,
                                                              seq_len)
        ...
        for i, town in enumerate(towns):
            ...
            for j, run_id in enumerate(town_paths):
                ...
                next_dist = min_dist_tiled + (
                        max_dist_tiled - min_dist_tiled) * np.random.random(
                    seq_len)
                while pano_indices[-1] < len(gnss_data):
                    ...
                    for k in range(1, seq_len):
                        ...
                        azimuth, bazimuth, distance = self.geod.inv(my_lng,
                                                                    my_lat,
                                                                    prev_pano_position[
                                                                        1],
                                                                    prev_pano_position[
                                                                        0])
                        while distance < next_dist[k] and pano_idx < len(
                                gnss_data):
                            pano_idx = pano_idx + 1
                            while pano_idx < len(
                                    gnss_data) and not self.pano_depth_file_exists(
                                data_path, town, run_id,
                                gnss_data[pano_idx]["frame"]):
                                pano_idx = pano_idx + 1
                    ...
                    if pano_indices[-1] < len(gnss_data):
                        ...
                        for k in range(seq_len):
                            ...
                            if not self.pano_depth_file_exists(data_path, town,
                                                               run_id,
                                                               frame):
                                save_set = False
                                break
                    ...
                    while pano_indices[0] < len(gnss_data) and \
                            not self.pano_depth_file_exists(
                                data_path, town, run_id,
                                gnss_data[pano_indices[0]]["frame"]):
                        pano_indices[0] = pano_indices[0] + 1
        ...
        for i, town in enumerate(towns):
            ...
            for j, run_id in enumerate(town_paths):
                ...
                if max_dist is not None:
                    next_dist = min_dist + (
                                max_dist - min_dist) * np.random.random()
    ...
    def load_custom_sequence(self, town, run_id, start_frame, data_path,
                             distance,
                             seq_len):
        pano_metadata = {}
        raw_paths = []
    ...
    def load_and_resize_panos(self, pano_ids):
        ...
        for i, pano_id in enumerate(pano_ids):
            ...
            rgb_pano_path = os.path.join(self.data_path, town, run_id, "rgb",
                                         frame + ".png")
            rgb_img = Image.open(rgb_pano_path).resize(
                (self.width, self.height))
            rgb_img = np.array(rgb_img, dtype=np.float32) / 255
            rgb_panos.append(rgb_img)
            depth_pano_path = os.path.join(self.data_path, town, run_id,
                                           "depth",
                                           frame + ".png")
        ...
        if self.use_euclidean_dist:
            depth_panos = self.zdepth_to_distance(
                depth_panos[:, :, :, np.newaxis])
    ...
    def distance_to_zdepth_torch(self, distance_image):
        ...
        zdepth_image = my_torch_utils.safe_divide(distance_image, ratio)
        return zdepth_image
    ...
    def cache_depth_to_dist(self, height, width):
        ...
        if cache_key not in self.depth_to_dist_cache:
            ...
            uvs, uv_sides = my_utils.spherical_to_cubemap(theta.reshape(-1),
                                                          phi.reshape(-1))
    ...
    def parse_location(self, location, dtype=np.float32):
        ...
        """
        return np.array([location["y"], location["z"], -location["x"]],
                        dtype=dtype)
    ...
    def calculate_rot_trans(self, paths, reference_idx=-1):
        ...
        for i, my_path in enumerate(paths):
            ...
            reference_location = self.parse_location(
                reference_metadata["location"])
            for j, pano_id in enumerate(my_path):
                if j is reference_idx:
                    ...
                    _, _, distance = self.geod.inv(my_lng, my_lat,
                                                   reference_lng,
                                                   reference_lat)
                    rot = (reference_rot * my_rot_inv).as_matrix()
                    t = location_delta
                    if distance > 0:
                        t = distance * location_delta / np.linalg.norm(
                            location_delta)
